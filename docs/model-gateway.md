...
> **APIM and the whole model gateway are always deployed.** The APIM Standard v2
> instance, its gateway spoke (`10.3.0.0/16`), inbound private endpoint,
> `privatelink.azure-api.net` DNS zone, the **provider Foundry** account, the APIM
> inference API/connection, and the second seeded agent are all provisioned
> unconditionally — both the model gateway *and* the Teams / M365 publish path route
> through this shared infrastructure.

The model gateway is deployed unconditionally in the shared gateway spoke
(`10.3.0.0/16`):
